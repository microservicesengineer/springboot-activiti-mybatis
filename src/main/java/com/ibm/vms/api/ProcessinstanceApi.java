...
public interface ProcessinstanceApi {
    ...
    @ApiOperation(value = "根据instance id挂起 / 激活实例", nickname = "changeInstanctStatusById", notes = "", response = StandardResponse.class, tags={ "Processinstance", })
    @ApiResponses(value = { 
    ...
    @RequestMapping(value = "/processInstance/historic/{id}",
        ...
        consumes = { "application/json" },
        method = RequestMethod.GET)
    default ResponseEntity<StandardResponse> _queryTaskHistoricalDataByID(@ApiParam(value = "",required=true) @PathVariable("id") String id) {
        return queryTaskHistoricalDataByID(id);
    }

    // Override this method
    default ResponseEntity<StandardResponse> queryTaskHistoricalDataByID(String id) {
        if(getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
            if (getAcceptHeader().get().contains("application/json")) {
                try {
                    return new ResponseEntity<>(getObjectMapper().get().readValue("{  \"code\" : 0,  \"data\" : \"{}\",  \"message\" : \"message\"}", StandardResponse.class), HttpStatus.NOT_IMPLEMENTED);
