...
public interface ProcessdefinitionApi {
    ...
    @ApiOperation(value = "根据de plodeploymentID获取可以撤回节点", nickname = "queryrecall", notes = "", response = StandardResponse.class, tags={ "Processdefinition", })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "", response = StandardResponse.class) })
    @RequestMapping(value = "/processDefinition/recall/{id}",
        produces = { "application/json" }, 
        consumes = { "application/json" },
        method = RequestMethod.GET)
    default ResponseEntity<StandardResponse> _queryrecall(@ApiParam(value = "",required=true) @PathVariable("id") String id) {
        return queryrecall(id);
    }

    // Override this method
    default ResponseEntity<StandardResponse> queryrecall(String id) {
        if(getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
            if (getAcceptHeader().get().contains("application/json")) {
                try {
                    return new ResponseEntity<>(getObjectMapper().get().readValue("{  \"code\" : 0,  \"data\" : \"{}\",  \"message\" : \"message\"}", StandardResponse.class), HttpStatus.NOT_IMPLEMENTED);
